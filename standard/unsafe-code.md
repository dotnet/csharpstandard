...
- An expression of type other than *array_type* or `string`, provided the type there exists an accessible method or accessible extension method named `GetPinnableReference`, that method has no parameters, that method returns a `ref` or `ref readonly` variable of an *unmanaged_type* `T`, and `T*` is implicitly convertible to the pointer type given in the `fixed` statement. The `GetPinnableReference`. In this case, the initializer computes the address of the returned variable, and that variable is guaranteed to remain at a fixed address for the duration of the `fixed` statement. A `GetPinnableReference()` method can be used by the `fixed` statement when overload resolution (§11.6.4) produces exactly one function member and that function member satisfies the preceding conditions. The `GetPinnableReference` method should return a reference to an address equal to zero, such as that returned from `System.Runtime.CompilerServices.Unsafe.NullRef<T>()` when there is no data to pin.
