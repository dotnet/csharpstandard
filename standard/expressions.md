...
-   A property access. Every property access has an associated type, namely the type of the property. Furthermore, a property access may have an associated instance expression. When an accessor (the `get` or `set` block) of an instance property access is invoked, the result of evaluating the instance expression becomes the instance represented by `this` ([§12.7.12](expressions.md#12712-this-access)).
-   An indexer access. Every indexer access has an associated type, namely the element type of the indexer. Furthermore, an indexer access has an associated instance expression and an associated argument list. When an accessor (the `get` or `set` block) of an indexer access is invoked, the result of evaluating the instance expression becomes the instance represented by `this` ([§12.7.12](expressions.md#12712-this-access)), and the result of evaluating the argument list becomes the parameter list of the invocation.
...
-   A namespace. An expression with this classification can only appear as the left-hand side of a *member_access* ([§12.7.6](expressions.md#1276-member-access)). In any other context, an expression classified as a namespace causes a compile-time error.
-   A type. An expression with this classification can only appear as the left-hand side of a *member_access* ([§12.7.6](expressions.md#1276-member-access)). In any other context, an expression classified as a type causes a compile-time error.
-   A method group, which is a set of overloaded methods resulting from a member lookup ([§12.5](expressions.md#125-member-lookup)). A method group may have an associated instance expression and an associated type argument list. When an instance method is invoked, the result of evaluating the instance expression becomes the instance represented by `this` ([§12.7.12](expressions.md#12712-this-access)). A method group is permitted in an *invocation_expression* ([§12.7.8](expressions.md#1278-invocation-expressions)) or a *delegate_creation_expression* ([§12.7.15.6](expressions.md#127156-delegate-creation-expressions)), and can be implicitly converted to a compatible delegate type ([§11.8](conversions.md#118-method-group-conversions)). In any other context, an expression classified as a method group causes a compile-time error.
-   An event access. Every event access has an associated type, namely the type of the event. Furthermore, an event access may have an associated instance expression. An event access may appear as the left-hand operand of the `+=` and `-=` operators ([§12.18.4](expressions.md#12184-event-assignment)). In any other context, an expression classified as an event access causes a compile-time error. When an accessor (the `add` or `remove` block) of an instance event access is invoked, the result of evaluating the instance expression becomes the instance represented by `this` ([§12.7.12](expressions.md#12712-this-access)).
...
A member lookup is the process whereby the meaning of a name in the context of a type is determined. A member lookup can occur as part of evaluating a *simple_name* ([§12.7.4](expressions.md#1274-simple-names)) or a *member_access* ([§12.7.6](expressions.md#1276-member-access)) in an expression. If the *simple_name* or *member_access* occurs as the *primary_expression* of an *invocation_expression* ([§12.7.8.2](expressions.md#12782-method-invocations)), the member is said to be *invoked*.
...
Type inference occurs as part of the binding-time processing of a method invocation ([§12.7.8.2](expressions.md#12782-method-invocations)) and takes place before the overload resolution step of the invocation. When a particular method group is specified in a method invocation, and no type arguments are specified as part of the method invocation, type inference is applied to each generic method in the method group. If type inference succeeds, then the inferred type arguments are used to determine the types of arguments for subsequent overload resolution. If overload resolution chooses a generic method as the one to invoke, then the inferred type arguments are used as the type arguments for the invocation. If type inference for a particular method fails, that method does not participate in overload resolution. The failure of type inference, in and of itself, does not cause a binding-time error. However, it often leads to a binding-time error when overload resolution then fails to find any applicable methods.
...
> ```
> The extension method invocation ([§12.7.8.3](expressions.md#12783-extension-method-invocations)) of `Select` is processed by rewriting the invocation to a static method invocation:
> ```csharp
...
-   Invocation of a method named in an *invocation_expression* ([§12.7.8](expressions.md#1278-invocation-expressions)).
-   Invocation of an instance constructor named in an *object_creation_expression* ([§12.7.15.2](expressions.md#127152-object-creation-expressions)).
-   Invocation of an indexer accessor through an *element_access* ([§12.7.10](expressions.md#12710-element-access)).
...
Each of these contexts defines the set of candidate function members and the list of arguments in its own unique way. For instance, the set of candidates for a method invocation does not include methods marked override ([§12.5](expressions.md#125-member-lookup)), and methods in a base class are not candidates if any method in a derived class is applicable ([§12.7.8.2](expressions.md#12782-method-invocations)).
...
-   For a delegate invocation ([§12.7.8.4](expressions.md#12784-delegate-invocations)), the list is a single function member with the same parameter list as the *delegate_type* of the invocation
-   For a method invocation ([§12.7.8.2](expressions.md#12782-method-invocations)) on a type, or on a value whose static type is not dynamic, the set of accessible methods in the method group is known at compile-time.
-   For an object creation expression ([§12.7.15.2](expressions.md#127152-object-creation-expressions)) the set of accessible constructors in the type is known at compile-time.
-   For an indexer access ([§12.7.10.3](expressions.md#127103-indexer-access)) the set of accessible indexers in the receiver is known at compile-time.
...
-   Instance function members. These are instance methods, instance constructors, instance property accessors, and indexer accessors. Instance function members are either non-virtual or virtual, and are always invoked on a particular instance. The instance is computed by an instance expression, and it becomes accessible within the function member as `this` ([§12.7.12](expressions.md#12712-this-access)). For an instance constructor, the instance expression is taken to be the newly allocated object.
...
- Otherwise:
  ...
  - The argument list is evaluated as described in [§12.6.2](expressions.md#1262-argument-lists).
  - If the type of `E` is a *value_type*, a boxing conversion ([§11.2.9](conversions.md#1129-boxing-conversions)) is performed to convert `E` to a *class_type*, and `E` is considered to be of that *class_type* in the following steps. If the *value_type* is an *enum_type*, the *class_type* is `System.Enum;` otherwise, it is `System.ValueType`.
...
The result of the invocation of an instance constructor ([§12.7.15.2](expressions.md#127152-object-creation-expressions)) is the value created. The result of the invocation of any other function member is the value, if any, returned ([§13.10.5](statements.md#13105-the-return-statement)) from its body.
...
### 12.7.3 Interpolated string expressions
...
An *interpolated_string_expression* is classified as a value. If it is immediately converted to `System.IFormattable` or `System.FormattableString` with an implicit interpolated string conversion ([§11.2.5](conversions.md#1125-implicit-interpolated-string-conversions)), the interpolated string expression has that type. Otherwise, it has the type `string`.
...
- Otherwise, for each instance type `T` ([§15.3.2](classes.md#1532-the-instance-type)), starting with the instance type of the immediately enclosing type declaration and continuing with the instance type of each enclosing class or struct declaration (if any):
  ...
  - Otherwise, if a member lookup ([§12.5](expressions.md#125-member-lookup)) of `I` in `T` with `e` type arguments produces a match:
    - If `T` is the instance type of the immediately enclosing class or struct type and the lookup identifies one or more methods, the result is a method group with an associated instance expression of `this`. If a type argument list was specified, it is used in calling a generic method ([§12.7.8.2](expressions.md#12782-method-invocations)).
    - Otherwise, if `T` is the instance type of the immediately enclosing class or struct type, if the lookup identifies an instance member, and if the reference occurs within the *block* of an instance constructor, an instance method, or an instance accessor ([§12.2.1](expressions.md#1221-general)), the result is the same as a member access ([§12.7.6](expressions.md#1276-member-access)) of the form `this.I`. This can only happen when `e` is zero.
    - Otherwise, the result is the same as a member access ([§12.7.6](expressions.md#1276-member-access)) of the form `T.I` or `T.I<A₁, ..., Aₑ>`.
...
### 12.7.5 Parenthesized expressions
...
-   Otherwise, an attempt is made to process `E.I` as an extension method invocation ([§12.7.8.3](expressions.md#12783-extension-method-invocations)). If this fails, `E.I` is an invalid member reference, and a binding-time error occurs.

#### 12.7.6.2 Identical simple names and type names

In a member access of the form `E.I`, if `E` is a single identifier, and if the meaning of `E` as a *simple_name* ([§12.7.4](expressions.md#1274-simple-names)) is a constant, field, property, local variable, or parameter with the same type as the meaning of `E` as a *type_name* ([§8.8.1](basic-concepts.md#881-general)), then both possible meanings of `E` are permitted. The member lookup of `E.I` is never ambiguous, since `I` shall necessarily be a member of the type `E` in both cases. In other words, the rule simply permits access to the static members and nested types of `E` where a compile-time error would otherwise have occurred. 
...
### 12.7.7 Null Conditional Member Access

A *null_conditional_member_access* is a conditional version of *member_access* ([§12.7.6](expressions.md#1276-member-access)) and it is a binding time error if the result type is `void`. For a null conditional expression where the result type may be `void` see ([§12.7.9](expressions.md#1279-null-conditional-invocation-expression)).
...
> P?.A₀?.A₁
> ```
then if `P` evaluates to `null` neither `A₀` or `A₁` are evaluated. The same is true if an expression is a sequence of *null_conditional_member_access* or *null_conditional_element_access* [§12.7.11](expressions.md#12711-null-conditional-element-access) operations. *end note*

A *null_conditional_projection_initializer* is a restriction of *null_conditional_member_access* and has the same semantics. It only occurs as a projection initializer in an anonymous object creation expression ([§12.7.15.7](expressions.md#127157-anonymous-object-creation-expressions)).

### 12.7.8 Invocation expressions
...
-   Otherwise, the result is a value, with an associated type of the return type of the method or delegate after any type argument substitutions ([§12.7.8.2](expressions.md#12782-method-invocations)) have been performed. If the invocation is of an instance method, and the receiver is of a class type `T`, the associated type is picked from the first declaration or override of the method found when starting with `T` and searching through its base classes.

#### 12.7.8.2 Method invocations
...
- If the resulting set of candidate methods is empty, then further processing along the following steps are abandoned, and instead an attempt is made to process the invocation as an extension method invocation ([§12.7.8.3](expressions.md#12783-extension-method-invocations)). If this fails, then no applicable methods exist, and a binding-time error occurs.
...
#### 12.7.8.3 Extension method invocations
...
### 12.7.9 Null Conditional Invocation Expression

A *null_conditional_invocation_expression* is syntactically either a *null_conditional_member_access* ([§12.7.7](expressions.md#1277-null-conditional-member-access)) or *null_conditional_element_access* ([§12.7.11](expressions.md#12711-null-conditional-element-access)) where the final *dependent_access* is an invocation expression ([§12.7.8](expressions.md#1278-invocation-expressions)).
...
If the *primary_no_array_creation_expression* of an *element_access* is a value of an *array_type*, the *element_access* is an array access ([§12.7.10.2](expressions.md#127102-array-access)). Otherwise, the *primary_no_array_creation_expression* shall be a variable or value of a class, struct, or interface type that has one or more indexer members, in which case the *element_access* is an indexer access ([§12.7.10.3](expressions.md#127103-indexer-access)).

#### 12.7.10.2 Array access
...
### 12.7.11 Null Conditional Element Access
...
A *null_conditional_element_access* is a conditional version of *element_access* ([§12.7.10](expressions.md#12710-element-access)) and it is a binding time error if the result type is `void`. For a null conditional expression where the result type may be `void` see ([§12.7.9](expressions.md#1279-null-conditional-invocation-expression)).
...
> P?[A₀]?[A₁]
> ```
if `P` evaluates to `null` neither `A₀` or `A₁` are evaluated. The same is true if an expression is a sequence of *null_conditional_element_access* or *null_conditional_member_access* [§12.7.7](expressions.md#1277-null-conditional-member-access) operations. *end note*
...
### 12.7.14 Postfix increment and decrement operators
...
#### 12.7.15.2 Object creation expressions
...
Processing of an object creation expression that includes an object initializer or collection initializer consists of first processing the instance constructor and then processing the member or element initializations specified by the object initializer ([§12.7.15.3](expressions.md#127153-object-initializers)) or collection initializer ([§12.7.15.4](expressions.md#127154-collection-initializers)).
...
#### 12.7.15.4 Collection initializers
...
#### 12.7.15.5 Array creation expressions
...
Implicitly typed array creation expressions can be combined with anonymous object initializers ([§12.7.15.7](expressions.md#127157-anonymous-object-creation-expressions)) to create anonymously typed data structures.
...
#### 12.7.15.6 Delegate creation expressions
...
#### 12.7.15.7 Anonymous object creation expressions
...
A member declarator can be abbreviated to a simple name ([§12.7.4](expressions.md#1274-simple-names)), a member access ([§12.7.6](expressions.md#1276-member-access)), a null conditional projection initializer [§12.7.7](expressions.md#1277-null-conditional-member-access) or a base access ([§12.7.13](expressions.md#12713-base-access)). This is called a ***projection initializer*** and is shorthand for a declaration of and assignment to a property with the same name. Specifically, member declarators of the forms
...
### 12.7.18 The checked and unchecked operators
...
-   The predefined `++` and `--` operators ([§12.7.14](expressions.md#12714-postfix-increment-and-decrement-operators) and [§12.8.6](expressions.md#1286-prefix-increment-and-decrement-operators)), when the operand is of an integral or enum type.
...
### 12.7.19 Default value expressions
...
Because `nameof` is not a keyword, a *nameof_expression* is always syntactically ambiguous with an invocation of the simple name `nameof`. For compatibility reasons, if a name lookup ([§12.7.4](expressions.md#1274-simple-names)) of the name `nameof` succeeds, the expression is treated as an *invocation_expression* -- regardless of whether the invocation is valid. Otherwise it is a *nameof_expression*.

Simple name and member access lookups are performed on the *named_entity* at compile time, following the rules described in [§12.7.4](expressions.md#1274-simple-names) and [§12.7.6](expressions.md#1276-member-access). However, where the lookup described in [§12.7.4](expressions.md#1274-simple-names) and [§12.7.6](expressions.md#1276-member-access) results in an error because an instance member was found in a static context, a *nameof_expression* produces no such error.
...
### 12.7.21 Anonymous method expressions
...
The result of evaluating `~x`, where `X` is an expression of an enumeration type `E` with an underlying type `U`, is exactly the same as evaluating `(E)(~(U)x)`, except that the conversion to `E` is always performed as if in an `unchecked` context ([§12.7.18](expressions.md#12718-the-checked-and-unchecked-operators)).
...
The `++` and `--` operators also support postfix notation ([§12.7.14](expressions.md#12714-postfix-increment-and-decrement-operators)). Typically, the result of `x++` or `x--` is the value of `X` before the operation, whereas the result of `++x` or `--x` is the value of `X` after the operation. In either case, `x` itself has the same value after the operation.
...
-   An identity ([§11.2.2](conversions.md#1122-identity-conversion)), implicit nullable ([§11.2.6](conversions.md#1126-implicit-nullable-conversions)), implicit reference ([§11.2.8](conversions.md#1128-implicit-reference-conversions)), boxing ([§11.2.9](conversions.md#1129-boxing-conversions)), explicit nullable ([§11.3.4](conversions.md#1134-explicit-nullable-conversions)), explicit reference ([§11.3.5](conversions.md#1135-explicit-reference-conversions)), or wrapping ([§9.3.11](types.md#9311-nullable-value-types)) conversion exists from `E` to `T`.
...
> *Note*: Because of [§12.7.6](expressions.md#1276-member-access), the same rule also applies to fields. *end note*
...
Unless a constant expression is explicitly placed in an `unchecked` context, overflows that occur in integral-type arithmetic operations and conversions during the compile-time evaluation of the expression always cause compile-time errors ([§12.7.18](expressions.md#12718-the-checked-and-unchecked-operators)).
