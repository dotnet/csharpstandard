...
When a formal parameter is an output parameter, the corresponding argument in a method invocation shall consist of the keyword `out`, optionally followed by *local_variable_type*,  followed by a *variable_reference* ([§10.5](variables.md#105-variable-references)) of the same type as the formal parameter, or declared `var`. A variable need not be definitely assigned before it can be passed as an output parameter, but following an invocation where a variable was passed as an output parameter, except in the case of a discard, the variable is considered definitely assigned.
