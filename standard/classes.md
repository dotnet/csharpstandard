...
- The declarations shall have the same modifiers (although not necessarily in the same order), with the exception of the `async` and `extern` modifiers which shall not appear on a defining part.
- In the case of private accessibility, both declarations shall be implicitly private (that is, with no accessibility modifier) or both shall be explicitly `private`.
